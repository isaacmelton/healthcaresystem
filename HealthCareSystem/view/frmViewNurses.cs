...
{
    public partial class frmViewNurses : Form
    {
        private List<User> users;
        private int selectedUser;

        public frmViewNurses()
        {
            InitializeComponent();
        }
        ...
        {
            users = NurseController.GetNursesList();
            lbUsers.Items.Clear();
            if (users.Count > 0)
            {
               ...
               if (MessageBox.Show("Are you sure you want to delete " + users[selectedUser].FullName, "Confirm Delete?", 
                    ...
                    MessageBoxDefaultButton.Button1) == System.Windows.Forms.DialogResult.Yes)
                {
                    try
                    {
                        ...
                        bool success = NurseController.DeleteNurseByID(users[selectedUser].PersonId);
                        if (success)
                        {
