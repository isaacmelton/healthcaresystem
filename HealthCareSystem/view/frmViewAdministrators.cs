...
{
    public partial class frmViewAdministrators : Form
    {
        private List<User> users;
        private int selectedUser;

        public frmViewAdministrators()
        {
            InitializeComponent();
        }
        ...
        {
            users = AdministratorController.GetAdministratorsList();
            lbUsers.Items.Clear();
            if (users.Count > 0)
            {
            ...
            if (lbUsers.SelectedIndex >= 0)
            {
                     ...
                     MessageBoxDefaultButton.Button1) == System.Windows.Forms.DialogResult.Yes)
                {
                    try
                    {
                        ...
                        bool success = AdministratorController.DeleteAdministratorByID(users[selectedUser].PersonId);
                        if (success)
                        {
